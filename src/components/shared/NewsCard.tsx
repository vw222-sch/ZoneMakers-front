...
export default function NewsCard({ title, image, description, url, publishDate, sourceCountry }: NewsCardProps) {
    ...
    return (
        <Card className='flex flex-col h-full max-w-sm overflow-hidden'>
            {image ? (
                <img
                    src={image}
                    alt={title}
                ...
                <div className="w-full aspect-video shrink-0 bg-muted/50 flex flex-col items-center justify-center text-muted-foreground">
            ...
            <CardHeader className='flex-1 space-y-2 pb-4'>
                ...
                <CardDescription className='line-clamp-3 text-muted-foreground text-pretty text-sm'>
                    {description}
                </CardDescription>
            </CardHeader>
